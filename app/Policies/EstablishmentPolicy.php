...
class EstablishmentPolicy
{
    ...
    public function view(User $user, Establishment $establishment)
    {
        if ($user->hasPermissionTo('view customers')) {
            return $establishment->customer->manutentors->contains('id', $user->id);
        }
     ...
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function restore(User $user, Establishment $establishment)
    {
        if ($user->hasPermissionTo('manage customers')) {
            return $establishment->customer->manutentors->contains('id', $user->id);
        }
     ...
     * @return \Illuminate\Auth\Access\Response|bool
     */
    public function forceDelete(User $user, Establishment $establishment)
    {
        if ($user->hasPermissionTo('manage customers')) {
            return $establishment->customer->manutentors->contains('id', $user->id);
        }

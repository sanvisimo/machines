...
class CreateControlPlansTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('control_plans', function (Blueprint $table) {
            ...
            $table->string('periodicity_note');
            $table->enum('global_conditions', ['good','attention','intervent']);
            $table->enum('machine_status',['run','stop', 'maintenance']);
            $table->boolean('casing_integrity_check');
            $table->string('casing_integrity_check_notes')->nullable();
            $table->boolean('nameplate_integrity');
